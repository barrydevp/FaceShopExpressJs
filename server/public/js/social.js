...
function createLiCommentText(comment, article) {
	...
	commentInnerText = '<li id="' + comment._id + '" class="id-' + comment._id + ' comment list-group-item"><div class="media"><a class="card-link" href="/user/view/' + (comment.author._id === user.id ? "" : comment.author._id) + '"><img class="rounded-circle mr-1" src="' + comment.author.avatar + '" width="35" alt="..."/></a><div class="media-body"><div class="form-group d-flex justify-content-between align-items-center"><div class="form-control" readonly="readonly"><a class="card-link font-weight-bold" href="/user/view/' + (comment.author._id === user.id ? "" : comment.author._id) + '">' + comment.author.fullname + ' </a><span>' + convertStringToTextHTML(comment.body) + '</span></div>' + dropdownCmt + '</div></div></div></li>';
...
function createLiCommentElement(comment, articleId) {
	...
	commentLiElement.innerHTML = '<div class="media"><a class="card-link" href="/user/view/"><img class="rounded-circle mr-1" src="' + user.avatar + '" width="35" alt="..."/></a><div class="media-body"><div class="form-group d-flex justify-content-between align-items-center"><div class="form-control" readonly="readonly"><a class="card-link font-weight-bold" href="/user/view/">' + user.fullname + ' </a><span>' + convertStringToTextHTML(comment.body) + '</span></div>' + dropdownCmt + '</div></div></div>';
...
function createCardArticleText(article) {
	...
	let dropdownHead = article.author._id !== user.id ? '' : ('<div><div class="dropdown"><button class="btn btn-link dropdown-toggle" id="gedf-drop1" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-right" aria-labelledby="gedf-drop1"><button id="' + article._id + '" class="edit-article btn btn-link dropdown-item" onclick="editArticle.bind(this)()">Edit</button><button id="' + article._id + '" class="del-article btn btn-link dropdown-item" onclick="deletePost.bind(this)()">Delete</button></div></div></div>');
		...
		let cardHeader = '<div class="card-header"><div class="d-flex justify-content-between align-items-center"><div class="d-flex justify-content-between align-items-center"><div class="mr-1"><a href="/user/view/' + (article.author._id == user.id ? "" : article.author._id) + '"><img class="rounded-circle" width="40" src="' + article.author.avatar + '" alt=""/></a></div><div class="ml-2"><div class="author-name font-weight-bold m-0"><a class="card-link" href="/user/view/' + (article.author._id === user.id ? "" : article.author._id) + '">' + article.author.fullname + '</a></div><div class="text-muted mb-2"><i class="far fa-clock"><span> ' + calculateTime(Date.now(), new Date(article.date)) + '</span></i></div></div></div>' + dropdownHead + '</div></div>';

...
function render(arrFriends) {
  ...
  arrFriends.forEach((friend, index) => {
    content += '<tr><td>' + (++index) + '</td><td style="width: 22%;">' + friend.fullname + '</td><td style="width: 22%;">' + friend.email + '</td><td>Pin Code</td><td>Country</td>';
...
async function deleteData() {
  const url = 'api/users/deletefriend/' + this.id;
  //console.log(url);
  let friends = [];
  try {

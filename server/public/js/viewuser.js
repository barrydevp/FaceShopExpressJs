...
async function unFriend() {
	...
	try {
		let url = '/api/users/delfr/' + friendId;
		//console.log(url);
		await axios.patch(url);
		let divReq = document.querySelector('.profiel-request');
		divReq.innerHTML = '<button class="btn btn-light mr-3" onclick="addRequestFriend()"><i class="fas fa-user-plus mr-1"></i><span>Add friend</span></button><button class="btn btn-light mr-1"> <i class="fas fa-external-link-square-alt mr-1"></i><span>Follow</span></button><button class="btn btn-light"><i class="fab fa-facebook-messenger mr-1"></i><span>Message</span></button>';
...
async function acceptRequestFriend() {
	...
	try {
		...
		liReq.remove();
		let divReq = document.querySelector('.profiel-request');
		divReq.innerHTML = '<button class="btn btn-light mr-3" onclick="unFriend()"><i class="fas fa-user-minus mr-1"></i><span>Unfriend</span></button><button class="btn btn-light mr-1"> <i class="fas fa-external-link-square-alt mr-1"></i><span>Follow</span></button><button class="btn btn-light"><i class="fab fa-facebook-messenger mr-1"></i><span>Message</span></button>';
	} catch(err) {
		console.error(err);
	}
} 
...
function createLiCommentText(comment, article) {
	...
	commentInnerText = '<li id="' + comment._id + '" class="id-' + comment._id + ' comment list-group-item"><div class="media"><a class="card-link" href="/user/view/' + (comment.author._id === userLogin.id ? "" : comment.author._id) + '"><img class="rounded-circle mr-1" src="' + comment.author.avatar + '" width="35" alt="..."/></a><div class="media-body"><div class="form-group d-flex justify-content-between align-items-center"><div class="form-control" readonly="readonly"><a class="card-link font-weight-bold" href="/user/view/' + (comment.author._id === userLogin.id ? "" : comment.author._id) + '">' + comment.author.fullname + ' </a><span>' + convertStringToTextHTML(comment.body) + '</span></div>' + dropdownCmt + '</div></div></div></li>';
...
function createLiCommentElement(comment, articleId) {
	...
	commentLiElement.innerHTML = '<div class="media"><a class="card-link" href="/user/view/"><img class="rounded-circle mr-1" src="' + userLogin.avatar + '" width="35" alt="..."/></a><div class="media-body"><div class="form-group d-flex justify-content-between align-items-center"><div class="form-control" readonly="readonly"><a class="card-link font-weight-bold" href="/user/view/">' + userLogin.fullname + ' </a><span>' + convertStringToTextHTML(comment.body) + '</span></div>' + dropdownCmt + '</div></div></div>';
...
function createCardArticleText(article) {
	...
	let dropdownHead = article.author._id !== userLogin.id ? '' : ('<div><div class="dropdown"><button class="btn btn-link dropdown-toggle" id="gedf-drop1" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-right" aria-labelledby="gedf-drop1"><button id="' + article._id + '" class="edit-article btn btn-link dropdown-item" onclick="editArticle.bind(this)()">Edit</button><button id="' + article._id + '" class="del-article btn btn-link dropdown-item" onclick="deletePost.bind(this)()">Delete</button></div></div></div>');
		...
		let cardHeader = '<div class="card-header"><div class="d-flex justify-content-between align-items-center"><div class="d-flex justify-content-between align-items-center"><div class="mr-1"><a href="/user/view/' + (article.author._id == userLogin.id ? "" : article.author._id) + '"><img class="rounded-circle" width="40" src="' + article.author.avatar + '" alt=""/></a></div><div class="ml-2"><div class="author-name font-weight-bold m-0"><a class="card-link" href="/user/view/' + (article.author._id === userLogin.id ? "" : article.author._id) + '">' + article.author.fullname + '</a></div><div class="text-muted mb-2"><i class="far fa-clock"><span> ' + calculateTime(Date.now(), new Date(article.date)) + '</span></i></div></div></div>' + dropdownHead + '</div></div>';
...
function createCardArticleElement(article) {
	...
	let dropdownHead = '<div><div class="dropdown"><button class="btn btn-link dropdown-toggle" id="gedf-drop1" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-right" aria-labelledby="gedf-drop1"><button id="' + article._id + '" class="edit-article btn btn-link dropdown-item" onclick="editArticle.bind(this)()">Edit</button><button id="' + article._id + '" class="del-article btn btn-link dropdown-item" onclick="deletePost.bind(this)()">Delete</button></div></div></div>';
		...
		let cardHeader = '<div class="card-header"><div class="d-flex justify-content-between align-items-center"><div class="d-flex justify-content-between align-items-center"><div class="mr-1"><a href="/user/view/"><img class="rounded-circle" width="40" src="' + userLogin.avatar + '" alt=""/></a></div><div class="ml-2"><div class="author-name font-weight-bold m-0"><a class="card-link" href="/user/view/">' + userLogin.fullname + '</a></div><div class="text-muted mb-2" ><i class="far fa-clock"><span> ' + calculateTime(Date.now(), new Date(article.date)) + '</span></i></div></div></div>' + dropdownHead + '</div></div>';
